...
export async function checkWebsite() {
  try {
    ...
    const currentStatus = getStatus() as any;
    let updates: any = {
    ...
    if (knownLinks.length === 0 && links.length > 0) {
      ...
      addLog('SYSTEM', 'Baseline established. Monitoring for new links.', `Saved ${links.length} initial links.`);
      return;
    }
    ...
    if (newLinks.length > 0) {
      ...
      for (const link of newLinks) {
        ...
        addLog('INFO', `New link detected: ${link.text}`, details);
        ...
        if (!currentStatus.admitCardReleased && checkKeywordsSingle(link, ADMIT_CARD_KEYWORDS)) {
          ...
          addLog('UPDATE', 'Admit Card release detected', details);
        }
        ...
        if (!currentStatus.responseSheetReleased && checkKeywordsSingle(link, RESPONSE_SHEET_KEYWORDS)) {
          ...
          addLog('UPDATE', 'Response Sheet release detected', details);
        }
        ...
        if (!currentStatus.resultReleased && checkKeywordsSingle(link, RESULT_KEYWORDS)) {
          ...
          addLog('UPDATE', 'Result declaration detected', details);
        }
      }
    ...
    if (!changesDetected && newLinks.length === 0) {
      addLog('CHECK', 'Website checked successfully, no new updates.');
    ...
    addLog('ERROR', 'Failed to check website', error.message);
  }
}
